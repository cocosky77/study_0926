...
/**
 * 상자를 클릭 -> 상자에 문자 "X" 입력 >> isXClick()
 * 9개 상자 각각의 id가 없음 >> useState를 이용해 각 상자의 value를 지정하기 위해 box 배열 제작
 * 각 상자 컴포넌트(<GameBox /> value에 box배열을 차례로 삽입함)
 * 
 * onBoxClick()를 onClick 메소드로 호출. [기존의 isXClick()와 연결시키기 위함]
 * => isXClick()은 한 상자에 X클릭을 보여주는 함수지만, 부모 컴포넌트(App)에서 자식 컴포넌트에
 * 일괄적으로 처리를 하기 위해 onBoxClick()을 만들어서 호출
 * 
 * 부모 컴포넌트(App)의 <GameBox />에서 onBoxClick()와 isXClick()을 연결
 * 
 * 기존 isXClick()와 다르게 수정된 isXClick()은 box[i]로 모든 상자를 업데이트할 수 있도록 수정
 * 
 * 
